...
<body>
    ...
    <header>
        ...
        <div class="flex justify-between px-2 laptop:px-10 py-2 bg-[#FAF4F6] items-center">
            <a href="{{ route('index') }}"> <img class="w-32 laptop:w-40 h-auto" src="{{ asset($setting->logo) }}"
                    alt=""></a>
            @php
                $categories = \App\Models\ProductCategory::with('subCategory')
                    ->select(['name', 'slug'])
                    ->where('status', 'active')
        ...
        @endphp --}}
            ...
            @php
                ...
                if (auth()->check()) {
                    ...
                    // Fetch cart items for guests based on guest_id
        ...
        <div class="container mx-auto grid gap-4 grid-cols-1 tablet:grid-cols-4">
            <div>
                <a href="{{ route('index') }}"><img
                        class="w-1/4 tablet:w-36 laptop:w-1/2  h-auto mb-2 tablet:mb-5 laptop:mb-6"
                ...
                <div>
                    @foreach ($socials as $social)
                        <a href="{{ $social->url }}" target="_blank">
                            <i
                                class="{{ $social->class }} text-red h-8 w-8 leading-[30px] text-center  border-red border-2 rounded-full hover:scale-105 ease-linear mr-2"></i>
                        </a>
                    @endforeach
                    <div class="mt-5">

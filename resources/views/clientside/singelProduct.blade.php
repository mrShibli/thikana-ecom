...
@section('client-content')
    ...
    <div class=" container mx-auto my-5 flex flex-wrap gap-4 p-3 ">
        <div class="w-full laptop:w-[430px] border rounded p-2">
            ...
            <div class="flex gap-1">
                ...
                <img class="w-60 laptop:w-80 object-contain" src="{{ asset($product->thumb_image) }}" alt="">
        ...
        <div class=" w-full laptop:w-[460px] border rounded p-3">
            ...
            <div class="mt-4">
                @if($product->offer)
                    ...
                    <h1 class="text-base tablet:text-xl laptop:text-2xl font-bold"><span class="text-red line-through">{{ $product->old_price }}৳</span>
                        <span id="updateOfferPrice">{{ $product->offer }}</span> ৳ </h1>
                @else
            ...
            <form action="{{ route('cart.store') }}" method="get">
                <div class="flex items-center gap-2 my-5">
                    ...
                    <input type="number" name="quantity" value="1" style="padding: 8px 0px"
                           class=" border text-black rounded   w-12"/>
                    <input type="hidden" name="product_id" value="{{ $product->id }}"/>
                    <input type="hidden" name="price" id="priceId" value="{{$product->offer??$product->old_price}}"/>
                    ...
                    <a href="#"
                       class=" py-2 px-8 laptop:py-3 laptop:px-10 bg-red rounded text-white text-sm tablet:text-base laptop:text-base font-bold">Buy
                        Now</a>
                </div>
            </form>
        </div>
        ...
        <div class="w-full laptop:w-[330px] ">
            ...
            <div class="p-4 border border-dashed border-blue">
                <p class="mb-3 text-xs laptop:text-sm"><i class="fa-solid fa-square-check text-sm text-blue mr-2"></i>Delivery
                ...
                <p class="mb-3 text-xs laptop:text-sm"><i class="fa-solid fa-square-check text-sm text-blue mr-2"></i>Product
                    হাতে পেয়ে মূল্য পরিশোধ।</p>
                <p class="mb-3 text-xs laptop:text-sm"><i class="fa-solid fa-square-check text-sm text-blue mr-2"></i>Product
                    পছন্দ না হলে সাথে সাথেই Return এর সুযোগ।</p>
                <p class="mb-3 text-xs laptop:text-sm"><i class="fa-solid fa-square-check text-sm text-blue mr-2"></i>Order
                    Confirmation এর ০২-০৪ দিনের ভিতর ডেলিভারী।</p>
                <p class="mb-3 text-xs laptop:text-sm"><i class="fa-solid fa-square-check text-sm text-blue mr-2"></i>Video
    ...
    <div class="deskcription-box container mx-auto py-10 p-3">
        <div class="flex justify-center items-center gap-2 border-b-2 border-gray-400 pb-1">
            <h1 class="description py-3 px-8 rounded bg-[#6B7280] font-medium text-white text-base cursor-pointer hover:bg-gray-600">
                Description</h1>
            <h1 class="Additional-information py-3 px-8 rounded bg-[#6B7280] font-medium text-white text-base cursor-pointer hover:bg-gray-600">
            ...
            <div id="additional_information" style="width: 100%">
                ...
                @foreach ($product->variations as $variations)
                    ...
                    <div class="gap-4 my-5" style="flex-wrap: nowrap; justify-content: center; align-items: center">
                        <p id="weight"
                           class="text-sm font-bold border-r border-r-slate-300 ">{{ $variations->name }} </p>
                        @foreach ($variations->options as $options)
                            ...
                            <div class="border-r border-r-slate-300"
                                 style="    background: #cbd5e1;height: 19px;"></div>
                            {{-- </div> --}}
                        @endforeach
                    </div>
                    <hr>
                ...
                <div class="swiper-slide text-center border rounded p-2 w-64 h-96">
                    <a href="{{ route('product.single', ['slug' => $slug, 'id' => $related_product->id]) }}">
                        <img class="bg-slate-100 p-2 w-full h-48 object-cover"
                             src="{{ asset($related_product->thumb_image) }}" alt="">
                    </a>
